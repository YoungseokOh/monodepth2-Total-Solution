...
def ncdl_to_pth(ckpt_path):
    
    loaded_dict = torch.load(ckpt_path)
    ckpt_dict = loaded_dict['state_dict']
    encoder_dict = OrderedDict()
    encoder = networks.ResnetEncoder(18, False)
    # [i for i in ckpt_dict if i in 'backbone']
    for k, v in ckpt_dict.items():
        ckpt_dict[k[9:]] = v
    encoder.load_state_dict({k[9:]: v for k, v in ckpt_dict.items() if 'encoder' in k[9:]})
...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        best_score_ckpt_name = 'scratch_epoch=19.ckpt'
        filenames = readlines(os.path.join(splits_dir, opt.eval_split, "test_files.txt"))
        ckpt_path = os.path.join(opt.load_weights_folder, best_score_ckpt_name)
        loaded_dict = torch.load(ckpt_path)
        ckpt_dict = loaded_dict['state_dict']
        ...
        if opt.depth_network == "DepthResNet":
            ...
            encoder.load_state_dict({k[9:]: v for k, v in ckpt_dict.items() if 'encoder' in k[9:]})
            depth_decoder.load_state_dict({k[5:]: v for k, v in ckpt_dict.items() if 'decoder' in k[5:]})
            if opt.decoder == "NCDL_Decoder":
                depth_head.load_state_dict({k[5:]: v for k, v in ckpt_dict.items() if 'moduleset' in k[5:]})
