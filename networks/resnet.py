...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1_rex = nn.Conv2d(3,
                               ...
                               stride=2,
                               bias=True)
        self.conv2_rex = nn.Conv2d(self.inplanes // 2,
                                   self.inplanes,
                                   kernel_size=2,
                                   stride=2,
                                   bias=True)
...
def lw_resnet(arch: str, block: Type[Union[BasicBlock,
                                         Bottleneck]], layers: List[int],
            pretrained: bool, progress: bool, **kwargs: Any) -> ResNet:
    model = LwResNet(block, layers, **kwargs)
    if pretrained:
        print('Lightweight ResNet-18 model')
        model_path = 'pretrained_model/LwDepthResNet/model_best_resnet-light.pth'
...
def lw_resnet18(pretrained: bool = False,
             progress: bool = True,
             **kwargs: Any) -> ResNet:
    ...
    """
    return lw_resnet('resnet18', BasicBlock, [2, 2, 2, 2], pretrained, progress,
                   **kwargs)
